 ...
 * time:2018.4.16
*/
#define ORDER_EVENTS_OPT__NUM_DETECTION_OPTS "num_detection_opts"
#define ORDER_EVENTS_OPT__MESSAGE_LENGTH "message_length"
#define ORDER_EVENTS_OPT__SID "sid"
/*********************************************************/
...
/***************************************************************************
 * add by liuhaitao. 2018.4.20
 * 对规则选项进行自定义排序
 **************************************************************************/
const int my_opts_num = 4;
char my_opts[20][50] = 
{
    "flags",
    "dsize",
...
int my_compare(const void *my_opt1, const void *my_opt2)
{

    char *my_opt11 = (char *)my_opt1;
    char *my_opt22 = (char *)my_opt2;
    int my_index1 = -1;
    int my_index2 = -1;
    for(int my_i = 0; my_i < my_opts_num; my_i++)
    {
        if( strncmp(my_opt11, my_opts[my_i], strlen(my_opts[my_i])) == 0)
        {
            my_index1 = my_i;
            break;
        }
    ...
    for(int my_i = 0; my_i < my_opts_num; my_i++)
    {
        if( strncmp(my_opt22, my_opts[my_i], strlen(my_opts[my_i])) == 0)
        {
            my_index2 = my_i;
            break;
        }
    ...
    if( my_index1 == -1 || my_index2 == -1)
        return 0;
    if( my_index1 > my_index2 )
        return 1;
    if( my_index1 < my_index2 )
        return -1;
    return 0;
...
char my_temp_str[1000][1000] = {};
void my_sort_rule_opts(char *s)
{
    char *t = s;
    int my_len = strlen(s);
    char *temp = s;
    ...
    while(*s != ')')
    {
        ...
        while(*s != ';')
        {
            my_temp_str[index2][index1++] = *s;
            *s++;
        }
        my_temp_str[index2][index1++] = ';';
        my_temp_str[index2][index1] = '\0';
        *s++;
        index1 = 0;
        index2++;
    }
    qsort(my_temp_str[0], index2, 1000, my_compare);
    char *p = t;
    *p++ = '(';
    for(int i = 0; i < index2; i++)
    {
        for(int j = 0; j < strlen(my_temp_str[i]); j++)
        {
            *p = my_temp_str[i][j];
            *p++;
        }
    ...
    for (i = 0; i < num_toks; i++)
    {
        ...
        else if (strcasecmp(toks[i], EVENT_QUEUE_OPT__ORDER_EVENTS) == 0)
        {
            ...
            if(i < num_toks)
            {
                ...
                /********************************************************************/
                /* add by liuhaitao; 2018.4.16 */
                else if (strcasecmp(toks[i], ORDER_EVENTS_OPT__MESSAGE_LENGTH) == 0)
                {
                    eq->order = SNORT_EVENTQ_MESSAGE_LEN;
                }
                else if (strcasecmp(toks[i], ORDER_EVENTS_OPT__NUM_DETECTION_OPTS) == 0)
                {
                    eq->order = SNORT_EVENTQ_NUM_DETECTION_OPTS;
                }
                else if (strcasecmp(toks[i], ORDER_EVENTS_OPT__SID) == 0)
                {
                    eq->order = SNORT_EVENTQ_SID;
                }
                /********************************************************************/
                else
                {
