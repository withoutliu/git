...
/***************************************************************************/
/*
*  add code by liuhaitao
*  time: 2018.4.16
*  in order to sort by other item 
*/

/* sort rules by message length */
static int sortOrderByMessageLength(const void *e1, const void *e2)
{
    OptTreeNode *otn1;
    OptTreeNode *otn2;
    if(!e1 || !e2)
    ...
    otn1 = *(OptTreeNode **)e1;
    otn2 = *(OptTreeNode **)e2;
    if( strlen(otn1->sigInfo.message) > strlen(otn2->sigInfo.message) )
        return 1;
    if( strlen(otn1->sigInfo.message) < strlen(otn2->sigInfo.message) )
        return -1;
...
/* sort rules by number options */

static int sortOrderByNumDetectionOpts(const void *e1, const void *e2)
{
    OptTreeNode *otn1;
    OptTreeNode *otn2;
    if(!e1 || !e2)
    ...
    if( otn1->num_detection_opts < otn2->num_detection_opts )
        return 1;
    if( otn1->num_detection_opts > otn2->num_detection_opts )
        return -1;
...
/* sort rules by sid */
static int sortOrderBySid(const void *e1, const void *e2)
{
    OptTreeNode *otn1;
    OptTreeNode *otn2;
    if( !e1 || !e2)
    ...
    if(otn1->sigInfo.otnKey.sid < otn2->sigInfo.otnKey.sid)
        return 1;
    if(otn1->sigInfo.otnKey.sid > otn2->sigInfo.otnKey.sid)
        return -1;
...
static inline int fpFinalSelectEvent(OTNX_MATCH_DATA *o, Packet *p)
{
    ...
    for( i = 0; i < o->iMatchInfoArraySize; i++ )
    {
        ...
        if(o->matchInfo[i].iMatchCount)
        {
            ...
            else if (eq->order == SNORT_EVENTQ_MESSAGE_LEN )
            {
                qsort(o->matchInfo[i].MatchArray, o->matchInfo[i].iMatchCount,
                     sizeof(void*), sortOrderByMessageLength);
            }
            else if (eq->order == SNORT_EVENTQ_NUM_DETECTION_OPTS )
            {
                qsort(o->matchInfo[i].MatchArray, o->matchInfo[i].iMatchCount,
                     sizeof(void*), sortOrderByNumDetectionOpts);
            }
            else if (eq->order == SNORT_EVENTQ_SID )
            {
                qsort(o->matchInfo[i].MatchArray, o->matchInfo[i].iMatchCount,
                     sizeof(void*), sortOrderBySid);
            }
            /************************************************************************/
            else
            {
